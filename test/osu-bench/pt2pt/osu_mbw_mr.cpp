...
double calc_bw(int rank, int size, int num_pairs, int window_size, char *s_buf,
        char *r_buf, mpl::communicator const& comm_world);
#else
double calc_bw(int rank, int size, int num_pairs, int window_size, char *s_buf,
        char *r_buf);
#endif
...
int main(int argc, char *argv[])
{
   ...
   if(window_varied) {
       ...
       for(j = 0, curr_size = 1; curr_size <= MAX_MSG_SIZE; curr_size *= 2, j++) {
           ...
           for(i = 0; i < WINDOW_SIZES_COUNT; i++) {
               ...
               bandwidth_results[j][i] = calc_bw(rank, curr_size, pairs,
                       window_array[i], s_buf, r_buf, comm_world);
#else
               bandwidth_results[j][i] = calc_bw(rank, curr_size, pairs,
       ...
       for(curr_size = 1; curr_size <= MAX_MSG_SIZE; curr_size *= 2) {
           double bw, rate;
#if defined(USE_MPL_CXX)
           bw = calc_bw(rank, curr_size, pairs, window_size, s_buf, r_buf, comm_world);
#else
...
#endif
{
    ...
    for(i = 0; i < size; i++) {
        ...
        mpl::contiguous_layout<char> l2(4);  
        mpl::irequest_pool rpool, spool;
        mpl::tag stag1 = mpl::tag(100);
        mpl::tag rtag1 = mpl::tag(101);
        mpl::tag stag2 = mpl::tag(101);
        mpl::tag rtag2 = mpl::tag(100);
#endif
    ...
    if(rank < num_pairs) {
...
#if defined(USE_MPL_CXX)
                spool.push(comm_world.isend(s_buf, l, target, stag1));
#else
...
#if defined(USE_MPL_CXX)
            spool.waitall();
            comm_world.recv(r_buf, l2, target, rtag1);
#else
        ...
        for(i = 0; i <  loop +  skip; i++) {
            ...
            for(j = 0; j < window_size; j++) {
#if defined(USE_MPL_CXX)
                rpool.push(comm_world.irecv(r_buf, l, target, rtag2));
#else
...
#if defined(USE_MPL_CXX)
            rpool.waitall();
            comm_world.send(s_buf, l2, target, stag2);
#else
            MPI_Waitall(window_size, request, reqstat);
